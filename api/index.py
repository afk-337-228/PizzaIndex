...
def fetch_pizzint_data():
    ...
    try:
        ...
        is_stale = data.get("data_freshness") == "stale"
        
        # If index is 0 but we have live data in the 'data' list, try to calculate a raw average
        if overall_index == 0 and "data" in data:
            popularities = [p.get("current_popularity", 0) for p in data.get("data", []) if p.get("current_popularity") is not None]
            if popularities:
                overall_index = round(sum(popularities) / len(popularities), 1)
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not stats:
            ...
            idx_display = f"{idx}" if idx > 0 else "0 (Waiting for updates)"
            if stats.get('stale'):
                idx_display += " [STALE]"
